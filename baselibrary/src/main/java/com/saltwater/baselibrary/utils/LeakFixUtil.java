 ...
 *     time   : 2018/12/03
 *     desc   : 修复InputMethodManager 导致的内存泄露
 *     version: 1.0
 * </pre>
 */
public class LeakFixUtil {
     ...
     * <p>
     * When a mView that has focus gets detached, we wait for the main thread to be idle and then
     * check if the InputMethodManager is leaking a mView. If yes, we tell it that the decor mView got
     * focus, which is what happens if you press home and come back from recent apps. This replaces
     * the reference to the detached mView with a reference to the decor mView.
     * <p>
                ...
                synchronized (lock) {
                    ...
                    if (servedView != null) {
                        ...
                        if (servedViewAttached) {
                            // The mView held by the IMM was replaced without a global focus change. Let's make
                            // sure we get notified when that mView detaches.
